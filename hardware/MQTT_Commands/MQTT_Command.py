...
Default value of MQTT_BUFFER_SIZE_BYTE is 512Users can change the max length limitation by setting MQTT_BUFFER_SIZE_BYTE in make menuconfigmax publish data length plus the MQTT header length(depends on topic name length) is equal to MQTT_BUFFER_SIZE_BYTE
